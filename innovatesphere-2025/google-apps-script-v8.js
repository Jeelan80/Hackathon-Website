// Google Apps Script for Hackathon Registration - V8
// Deploy this as a Web App to receive form submissions - With Payment Screenshot Support
  ...
  try {
    ...
    if (e.postData && e.postData.contents) {
      ...
      try {
        ...
        console.log('Error parsing postData.contents directly:', error);
        
        try {
          // Try to extract data parameter from form submission
          const urlParams = new URLSearchParams(e.postData.contents);
          const jsonData = urlParams.get('data');
          
          if (jsonData) {
            console.log('Found data parameter in URL params');
            data = JSON.parse(jsonData);
            console.log('Successfully parsed extracted data parameter');
          } else {
            console.log('No data parameter found in URL params');
            throw new Error('No data parameter found');
          }
        } catch (parseError) {
          console.log('Error parsing URL params:', parseError);
          throw new Error('Failed to parse URL-encoded data: ' + parseError);
        }
      }
      ...
      try {
        ...
        console.log('Error parsing e.parameter.data:', error);
        throw new Error('Invalid JSON in parameter.data: ' + error);
      }
    } else {
      console.log('No recognized data format found');
